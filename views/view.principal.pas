...
begin
  LoadEvents(Calendar.CalendarDate); // carrega eventos para a data selecionada
end;

procedure TViewPrincipal.edtDescriptionClick(Sender: TObject);
begin // limpa o edt quando o usuário clicar no campo
    if edtDescription.Text = 'Digite a descrição da sua tarefa...' then
       edtDescription.Text := '';
end;
...
begin
  if edtTitle.Text = 'Digite o título do seu evento...' then
     edtTitle.Text := '';
end;
...
begin
  ...
  // exibe os eventos na ListBox
  while not FDQuery.Eof do
  begin
    // adiciona infos ao primeiro listbox
    ListBoxEvents.Items.Add(FDQuery.FieldByName('age_titulo').AsString +
                            ' - ' + DateToStr(FDQuery.FieldByName('age_data').AsDateTime) +
                            ' às ' + TimeToStr(FDQuery.FieldByName('age_hora').AsDateTime));
